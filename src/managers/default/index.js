...
class DefaultViewManager {
	...
	render(element, size) {
		...
		if (this.settings.fullsize === undefined &&
	...
	onOrientationChange(e) {
		...
		if (this.options.settings.resizeOnOrientationChange) {
			this.resize();
		}
	...
	prev() {
		...
		if (prev) {
			...
			let forceRight = false;
			if (this.layout.name === "pre-paginated" && 
				this.layout.divisor === 2 && typeof prev.prev() !== "object") {
				forceRight = true;
			}
	...
	visible() {
		...
		for (let i = 0; i < viewsLength; i++) {
			const view = views[i];
			const isVisible = this.isVisible(view, 0, 0, container);
